...
Приложение развернуто на европейских серверах (Frankfurt).  
В некоторых регионах (включая РФ) доступ может быть ограничен на уровне сети.

Если приложение не открывается, рекомендуется использовать VPN  
или развернуть проект локально (см. раздел **Local Setup**).

---

## 🧪 Демо-аккаунт

Email: [demo@probur.dev](mailto:demo@probur.dev)
